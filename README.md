...
Создан класс PointStream, который содержит статический метод collectPolyline() служит для построения ломанной линии. Реализован следующий алгоритм:
1. Создание потока points.stream().
...
7. Вывод информации о классе: возвращается исходный список точек в виде "Точки: [{x1;y1}, {x2;y2}, ..., {xn;yn}]" и построенная ломанная линия в виде "Ломанная: Линия [{x1;y1}, {x2;y2}, ..., {xn;yn}]". Если исходный список пустой метод выводит строку "Список точек пустой. Невозможно создать линию." и завершает работу.
...
### Алгоритм решения
Создан класс HumanStream cо статическим методом collectionHuman(File file) используется для обработки данных из файла и их группировки по числовым значениям:
...
6. Вывод списка исходных точек и получившейся ломанной линии.
...
1. IllegalArgumentException - метод valFile из класса Validator
   ...
   -  Файл не найден (метод exists() из класса File проверяет существет ли указанный файл в системе)
   -  Файл пустой (метод length() из класса File возвращает размер файла и сравнивает с нулём)
2. RuntimeException - метод collectionHuman()
   - Ошибка чтения файла (например, передан не файл, а директория)
