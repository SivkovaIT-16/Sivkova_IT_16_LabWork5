...
result - хранит данные победителей (участников с максимальным баллом). Добавлены методы доступа: getInput(), getResult(), setInput(). Конструктор по умолчанию инициализирует пустые исходный и результирующий контейнеры. Конструктор с параметром принимает файл, метод valFile() из класса Validator проверяет файл перед обработкой, далее происходит чтение данных из файла с помощью приватного метода readFile() и определение победителей с помощью метода selection(). Метод toString() проверяет контейнеры на пустоту, если пустые, выводит соответствующее сообщение и завершает работу. Иначе формирует и выводит списки всех участников и победителей.
