...
### Алгоритм решения
Создан класс ClassList с приватными полями list - исходный список чисел для обработки и result - результирующий список после применения операции удаления, списки хранят значения типа Number. Добавлены геттеры getList() и getResult() для получения данных и сеттер setList() для установки данных (с проверкой на null). Конструктор по умолчанию инициализирует пустые исходный и результирующий списки. Конструктор с параметрами принимает исходный список, проверяет его на значение null с помощью метода validateNull() из класса ru.sivkova.validator.Validator, при успешной валидации, сохраняет исходный список в поле list и инициализирует пустой список для результатов. Метод toString() предоставляет строковое представление объекта ClassList, отображая исходные данные и результаты операции удаления.
...
Класс ru.sivkova.validator.Validator содержит следующие методы:
...
result - хранит данные победителей (участников с максимальным баллом). Добавлены геттеры getInput() и getResult(), для получения данных и сеттер setInput() для установки данных (с проверкой на null). Конструктор по умолчанию инициализирует пустые исходный и результирующий контейнеры. Конструктор с параметром принимает файл, метод valFile() из класса ru.sivkova.validator.Validator проверяет файл перед обработкой, далее происходит чтение данных из файла с помощью приватного метода readFile() и определение победителей с помощью метода selection(). Метод toString() проверяет контейнеры на пустоту, если пустые, выводит соответствующее сообщение и завершает работу. Иначе формирует и выводит списки всех участников и победителей.
...
7. Проверка числового формата и диапазона баллов (0-25), преобразование строк в числа (используется valIsNumber() из класса ru.sivkova.validator.Validator для проверки что баллы - числа, при ошибке - сообщение и переход к следующей строке);
...
1. IllegalArgumentException - методы valFile() и validateNullMap() из класса ru.sivkova.validator.Validator 
...
3. NumberFormatException 
   ...
   - В методе ru.sivkova.validator.Validator.valIsNumber(): баллы участников содержат нечисловые значения (возвращает false)
...
4. Первая строка должна содержать количество участников.
5. Файл содержит меньше строк, чем указано.
6. Количество участников превышает 250.
7. Ошибка: Файл не может быть null.
8. Ошибка: Файл Map.txt не найден.
9. Ошибка: Файл Empty.txt пустой.
10. Ошибка чтения файла D:\IdeaProjects.
...
### Алгоритм решения
Класс ClassSet содержит следующие private поля: text (String) - хранит исходный текст для анализа, letters (Set<Character>)  - хранит результирующее множество согласных букв, встречающихся ровно в одном слове. Добавлены методы доступа: getText(), getLetters(), setText(). Конструктор по умолчанию инициализирует text пустой строкой, создает пустое множество letters. Конструктор с параметром принимает файл, метод valFile() из класса ru.sivkova.validator.Validator проверяет файл перед обработкой, далее происходит попытка чтения содержимого файла в поле text с помощью метода Files.readString() (при ошибке чтения, выбрасывает исключение с соответствующим сообщением), вызывается метод Text() для поиска согласных букв, входящих ровно в одно слово. Метод toString() проверяет множетство letters на пустоту, если множество пустое, возвращает соответсвующее сообщение, иначе выводит строковое представление множества letters.
...
Программа обрабатывает следующие исключения:
1. IllegalArgumentException - метод valFile из класса ru.sivkova.validator.Validator
   - Некорректные входные данные (когда передается null вместо файла)
   -  Файл не найден (метод exists() из класса File проверяет существет ли указанный файл в системе)
   -  Файл пустой (метод length() из класса File возвращает размер файла и сравнивает с нулём)
...
Дан набор объектов типа Point, необходимо взять все ru.sivkova.point.Point в разных координатах, убрать с одинаковыми X,Y, отсортировать по X, отрицательные Y сделать положительными и собрать это все в ломаную (объект типа ru.sivkova.point.Polyline)
Если раннее в вашем варианте не было задание с классом ru.sivkova.point.Point и ru.sivkova.point.Polyline, то написать их:
1. класс Point:
  - Координата Х: число.
  - Координата Y: число.
